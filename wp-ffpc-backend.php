...
if (!class_exists('WP_FFPC_Backend')) :
    ...
    class WP_FFPC_Backend
    {
        ...
        {
            // prevent this from running multiple times in one run
//            if (did_action('transition_post_status'))
//                return;
//            echo 'clear once' . PHP_EOL;
//            echo current_filter();
            ...
            // purge author page
            $to_clear[] = $this->get_post_author_url($post_id);

            // purge front page 
            $to_clear[] = $this->get_site_url();
...
//            file_put_contents('/tmp/to_clear', print_r($to_clear, true));
//            echo 'salam';
            /* run clear */
        ...
        public function add_feed_path_to_urls(&$urls)
        {
        ...
        public function clear_by_post_url($url)
        {
        ...
        public function clear_by_post_id($post_id)
        {
...
//                                                                                print_r($links);
//                    echo "there are ." . count($links) . " links";
//                                exit();
                }
            }
        ...
        private function memcached_clear(&$keys)
        {
            foreach ((array) $keys as $key) {
                ...
                foreach (array($this->options['prefix_meta'], $this->options['prefix_data']) as $prefix) {

                    $item = $prefix . $key;
                    $kresult = $this->connection->delete($item);
